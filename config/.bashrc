...
__conda_setup="$('/home/esteban/anaconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
...
. "$HOME/.cargo/env"
source /home/esteban/alacritty/extra/completions/alacritty.bash

# Prevents a command from being saved in the command history by using the
# HISTCONTROL environment variable.
nohistory() { HISTCONTROL=ignoreboth; "$@"; unset HISTCONTROL; }
...
# Prevents a command from being saved in the command history by using the
# HISTCONTROL environment variable.
no_history() { HISTCONTROL=ignoreboth; "$@"; unset HISTCONTROL; }
